...
/* **********************************************************************
 * getCivicLedgerLedgerState: a helper that queries the current state of
...
export const getCivicLedgerLedgerState = async (
...
const deployOrJoin = async (providers: CivicLedgerProviders, rli: Interface, logger: Logger): Promise<CivicLedgerAPI | null> => {
  let api: CivicLedgerAPI | null = null;

  while (true) {
    const choice = await rli.question(DEPLOY_OR_JOIN_QUESTION);
    switch (choice) {
      case '1':
        api = await CivicLedgerAPI.deploy(providers, logger);
        logger.info(`Deployed contract at address: ${api.deployedContractAddress}`);
        return api;
      case '2':
        api = await CivicLedgerAPI.join(providers, await rli.question('What is the contract address (in hex)? '), logger);
...
const displayLedgerState = async (
  providers: CivicLedgerProviders,
  deployedCivicLedgerContract: DeployedCivicLedgerContract,
  logger: Logger,
): Promise<void> => {
  const contractAddress = deployedCivicLedgerContract.deployTxData.public.contractAddress;
  const ledgerState = await getCivicLedgerLedgerState(providers, contractAddress);
  if (ledgerState === null) {
...
const displayPrivateState = async (providers: CivicLedgerProviders, logger: Logger): Promise<void> => {
  const privateState = await providers.privateStateProvider.get(CivicLedgerPrivateStateKey);
...
const displayDerivedState = (ledgerState: CivicLedgerDerivedState | undefined, logger: Logger) => {
...
const mainLoop = async (providers: CivicLedgerProviders, rli: Interface, logger: Logger): Promise<void> => {
  const CivicLedgerApi = await deployOrJoin(providers, rli, logger);
  if (CivicLedgerApi === null) {
    return;
  }
  let currentState: CivicLedgerDerivedState | undefined;
  const stateObserver = {
    next: (state: CivicLedgerDerivedState) => (currentState = state),
  };
  const subscription = CivicLedgerApi.state$.subscribe(stateObserver);
  try {
    while (true) {
      ...
      try {
        switch (choice) {
          case '1': {
            ...
            await CivicLedgerApi.post(message);
            break;
          }
          case '2':
            await CivicLedgerApi.takeDown();
            break;
          case '3':
            await displayLedgerState(providers, CivicLedgerApi.deployedContract, logger);
            break;
          case '4':
